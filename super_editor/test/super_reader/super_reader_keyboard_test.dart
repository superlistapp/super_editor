...
void main() {
  group('SuperReader keyboard', () {
    ...
    testAllInputsOnMacAndIos("and removes selection when it collapses without holding the SHIFT key", (
      tester, {
      required TextInputSource inputSource,
    }) async {
      final nodeId = await _pumpSingleLineAndSelectAWord(tester, offset: 10, inputSource: inputSource);
    ...
    testAllInputsOnMacAndIos("and retains the selection when collapsed and the SHIFT key is pressed", (
      tester, {
      required TextInputSource inputSource,
    }) async {
      final nodeId = await _pumpSingleLineAndSelectAWord(tester, offset: 10, inputSource: inputSource);
