...
class DeleteUpstreamAtBeginningOfTaskCommand implements EditCommand {
  DeleteUpstreamAtBeginningOfTaskCommand(this.node);
  ...
  void execute(EditContext context, CommandExecutor executor) {
    ...
    if (nodeBefore is TextNode) {
      // The caret is at the beginning of one TaskNode and is preceded by
      // a TextNode. Merge the two TextNodes.
    ...
    if ((node as TextNode).text.text.isEmpty) {
      // The caret is at the beginning of an empty TaskNode and the preceding
      // node is not a TextNode. Delete the current TaskNode and move the
